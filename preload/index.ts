...
const IPC_CHANNELS = {
  APP: { GET_VERSION: 'app:get-version', ON_CLI_ARGS: 'app:on-cli-args', GET_INITIAL_CLI_ARGS: 'app:get-initial-cli-args', SET_STARTUP: 'app:set-startup', GET_STARTUP: 'app:get-startup', SHOW_NOTIFICATION: 'app:show-notification', NOTIFICATION_CLICKED: 'app:notification-clicked', REG_CONTEXT_MENU: 'app:reg-context-menu', UNREG_CONTEXT_MENU: 'app:unreg-context-menu', GET_CONTEXT_MENU_STATUS: 'app:get-context-menu-status' },
  ...
  UPDATER: { CHECK_FOR_UPDATES: 'updater:check', QUIT_AND_INSTALL: 'updater:quit-and-install', ON_UPDATE_AVAILABLE: 'updater:available', ON_UPDATE_NOT_AVAILABLE: 'updater:not-available', ON_UPDATE_DOWNLOADED: 'updater:downloaded', ON_UPDATE_ERROR: 'updater:error' },
...
const electronAPI = {
  app: {
    ...
    getInitialCLIArgs: () => ipcRenderer.invoke(IPC_CHANNELS.APP.GET_INITIAL_CLI_ARGS),
  ...
  updater: {
    ...
    onUpdateNotAvailable: (callback: (info: any) => void) => {
      const listener = (_event: any, info: any) => callback(info);
      ipcRenderer.on(IPC_CHANNELS.UPDATER.ON_UPDATE_NOT_AVAILABLE, listener);
      return () => ipcRenderer.removeListener(IPC_CHANNELS.UPDATER.ON_UPDATE_NOT_AVAILABLE, listener);
    },
    ...
    onUpdateError: (callback: (message: string) => void) => {
      ...
      ipcRenderer.on(IPC_CHANNELS.UPDATER.ON_UPDATE_ERROR, listener);
      return () => ipcRenderer.removeListener(IPC_CHANNELS.UPDATER.ON_UPDATE_ERROR, listener);
    },
  },
  notifications: {
